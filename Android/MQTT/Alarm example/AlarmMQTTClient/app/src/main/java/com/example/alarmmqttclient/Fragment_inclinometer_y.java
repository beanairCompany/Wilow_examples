 ...
 * Use the {@link Fragment_inclinometer_y#newInstance} factory method to
 * create an instance of this fragment.
 */
public class Fragment_inclinometer_y extends Fragment {

    TextView tv_alarm_incY;
    LineChart chart;
    long tsLong;
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if(isVisibleToUser){
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    final String ChanneTopic = MqttClient_alarm.deviceId+"/SENSOR/4";
                    tv_alarm_incY =  (TextView) getView().findViewById(R.id.tv_alarm_incY);
                        ...
                        public void messageArrived(String topic, MqttMessage mqttMessage) throws UnsupportedEncodingException {
                            if (isAlarm(mqttMessage.getPayload())) {
                                if(topic.equals(ChanneTopic)) {
                                    tv_alarm_incY.setText(get_alarmStatus(mqttMessage.getPayload()));

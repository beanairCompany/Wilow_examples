 ...
 *Reference
 *docuement http://www.wireless-iot.beanair.com/files/TN-RF-004-MQTT-Comnmunication-Protocol.pdf (pages: 49)
 *Preparation:
 ...
 * @make sure to subscripe to an existing topic, mean the channel is active.
...
typedef enum Packet_Low_Duty_Cycle{DEVICE_TYPE,DAC_TYPE ,CHANNEL_ID ,REFERENCE_TIME ,DATA_SAMPLE=7} Packet_Low_Duty_Cycle_t; 
...
void VDetectDataFromFrame(const char *p_INPUT ,unsigned char *p_output,const unsigned char  LENGTH,Packet_Low_Duty_Cycle_t byteIndex);
...
{   /**@Slicing the upcoming frame**/
    ...
    if(dataAqcType==LOW_DUTY_CYCLE) 
    {
        /**@Collect data measurments from a BeanDevice in Alarm mode**/
        printf("**************************************************************************\n");
...
unsigned char u8DetectDeviceType(const unsigned char BenDevice)
{
  /**@Detecting the device type and print it **/
  switch (BenDevice)
     {
        case X_3D:
...
{
   /****@Detect the sample measured from the frame and print it.
...
void VDetectDataFromFrame(const char *p_INPUT ,unsigned char *p_output,const unsigned char  LENGTH,Packet_Low_Duty_Cycle_t byteIndex)
{
   /*Detect the data from the frame and print it*/
    unsigned char temp=0;

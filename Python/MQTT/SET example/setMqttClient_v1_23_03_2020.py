...
Author: Habib jomaa
Description:
This Example uses MQTT protocol to collect data measurments from a BeanDevice in S.E.T mode and display it in graph. 
...
Informations we need:
    S.E.T frame content:
+(0)---------+(1)------+(2)-------+(8)----------------+(10)------------+(14)--------------+(17)-----------------------+(19)-------------+(22)---------+(25)--------------------------------+(27)-----+(28)-----+(29)---------+(30)-----+(30+n)
|Device type |DAC type |time      |sampling frequency |channels bitmap |frame sequence id |number of data per channel |Data Notif cycle |DAC duration |previouse number of dac per channel |flags    |LOI      |Alarm Status |Data     |
|(1 byte)    |(1 byte) |(6 bytes) |(2 bytes)          |(4 bytes)       |(3 bytes)         |(2 bytes)                  |(3 bytes)        |(3 bytes)    |(2 bytes)                           |(1 byte) |(1 byte) |(1 byte)     |(n bytes)| 
+------------+---------+----------+-------------------+----------------+------------------+---------------------------+-----------------+-------------+------------------------------------+---------+---------+-------------+---------+
...
Reference
docuement http://www.wireless-iot.beanair.com/files/TN-RF-004-MQTT-Comnmunication-Protocol.pdf (pages: 46, 50)

Preparation:
1- Configure the beandevice to use the MQTT mode
2- Make sure to enable the topic for dynamic measurement
3- Start running the beandevice in S.E.T mode

Steps:
1- connect to the broker
2- subscribe to the topic "STREAMING" (it is the same topic for streaming and S.E.T mode) to receive tx messages
3- subscribe to the topic "UPDATE" to receive the new profile informations at the end of the streaming. (we will use it only to keep track of the end of stream)
    (the alternative way is to use the frame id, if it's 0 it mean it's a new streaming {see SSD example})
4- wait for payloads
5- check DAC type if it's S.E.T
6- parse the coming payload
7- display data in graph
8- repeate steps (4->7) until an update message received
9- clear graph on the new stream
'''
...
### Because we will listen only to the SET mode DAC we will need only the topic for streaming mode (it's the same topic for the Streaming S.E.T and SSD mode)
setTopic = deviceMacId+"/STREAMING"
### the update topic
updateTopic = deviceMacId+"/UPDATE"
## graph
...
def get_data(payload, channels, nbrDataPerChannel, dateTimeArray):
    ...
    for i in range(nbrChannel):
        ...
        for j in range(nbrDataPerChannel):
            ...
            #data in 3 byte (LSB first) and it's signed data (last bit is a sign bit)
            value = 0x000000
            value |= payload[pos]
            value |= (payload[pos+1]<<8) 
            value |= (payload[pos+2]&0x7f)<<16
            data = int(value)/1000
            #if sign bit equal 1 the measurement is negative
            if((payload[pos+2:pos+3][0]&0x80)==0x80):
                data *= -1
            # save data (time in utc time format)
            channelsData[i][1][j][0]=datetime.datetime.utcfromtimestamp(dateTimeArray[j]).strftime('%Y-%m-%d %H:%M:%S.%f')
            channelsData[i][1][j][1]=data
    return channelsData
...
def parsePayloadBeandevice(payload):
    ...
    channelsData = get_data(payload[30:], channels, nbrSamplePerChannel, dateTimeArray)
    return deviceType, deviceDACmode, synchronizationStatus, channelsData


# check if DAC mode of the device is streaming
def isDeviceDacModeSet(payload):
    deviceDacMode = get_deviceDacMode(payload[1])
    if(deviceDacMode=="S.E.T"):
        return True
    return False 
...
def on_connect(client, userdata, flags, rc):
    ...
    print("Subscription to STREAMING and UPDATE topics, done!")

# callback function for the upcoming messages
def on_message(client, userdata, msg):
    global oldTimeArray, oldDataArray
    # check if the message come from "STREAMIN" or "UPDATE" topic
    if(msg.topic == setTopic):
        # check if the payload of the message come from a S.E.T DAC mode
        if(isDeviceDacModeSet(msg.payload)):
            deviceType, deviceDACmode, synchronizationStatus, channelsData = parsePayloadBeandevice(msg.payload)
    ...
    # if message comes from update topic so it's the end of the stream period, cleare the stored time and values
    elif(msg.topic == updateTopic):
        oldTimeArray = [[] for x in range(5)]
        oldDataArray = [[] for x in range(5)]
...
########### Main section ###########
####################################
print("Start the MQTT S.E.T client")
## MQTT

...
+(0)---------+(1)------+(2)-------+(8)----------------+(10)------------+(14)--------------+(17)-----------------------+(19)------+(22)---------+(25)--------------------------------+(27)-----+(28)-----+(29)-----+(29+n)
|Device type |DAC type |time      |sampling frequency |channels bitmap |frame sequence id |number of data per channel |DAC cycle |DAC duration |previouse number of dac per channel |flags    |LOI      |Data     |
|(1 byte)    |(1 byte) |(6 bytes) |(2 bytes)          |(4 bytes)       |(3 bytes)         |(2 bytes)                  |(3 bytes) |(3 bytes)    |(2 bytes)                           |(1 byte) |(1 byte) |(n bytes)| 
+------------+---------+----------+-------------------+----------------+------------------+---------------------------+----------+-------------+------------------------------------+---------+---------+---------+
...
Reference
docuement http://www.wireless-iot.beanair.com/files/TN-RF-004-MQTT-Comnmunication-Protocol.pdf (pages: 46, 48)
...
### Because we will listen only to the streaming mode DAC we will need only the topic for streaming mode (it's the same topic the Streaming S.E.T and SSD mode)
streamingTopic = deviceMacId+"/STREAMING"
## graph
...
def parsePayloadBeandevice(payload):
    ...
    channelsData = get_data(payload[29:], channels, nbrSamplePerChannel, dateTimeArray)
    return deviceType, deviceDACmode, synchronizationStatus, channelsData, frameId

# check if DAC mode of the device is streaming
def isDeviceDacModeStream(payload):
    deviceDacMode = get_deviceDacMode(payload[1])
    if(deviceDacMode=="Streaming"):
        return True
    return False 
...
def on_connect(client, userdata, flags, rc):
    ...
    client.subscribe(streamingTopic)
...
def on_message(client, userdata, msg):
    ...
    # check if the playload of the message come from a streaming DAC mode
    if(isDeviceDacModeStream(msg.payload)):
        deviceType, deviceDACmode, synchronizationStatus, channelsData, frameId = parsePayloadBeandevice(msg.payload)
        # if it's a new stream, clear stored data
        if(frameId==0):
            oldTimeArray = [[] for x in range(5)]
            oldDataArray = [[] for x in range(5)]
...
########### Main section ###########
####################################
print("Start the MQTT Streaming client")
## MQTT

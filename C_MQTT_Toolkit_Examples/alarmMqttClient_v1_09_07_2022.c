 ...
 *
 * Description:
 * This Example uses MQTT protocol to collect data measurments from a BeanDevice in Alarm  mode
 * and display it in screen along with the alarm status for each channel.   
 ...
 *
 * Reference
 * docuement http://www.wireless-iot.beanair.com/files/TN-RF-004-MQTT-Comnmunication-Protocol.pdf (pages: 49, 50)
 * Preparation:
 ...
 * 2- subscribe to each sensor's topic (static measurment topics){[MAC_ID]/Sensor0, [MAC_ID]/Sensor1 ... etc}.
 * 3- wait for payloads
 * 4- check DAC type if it's Alarm
 * 5- parse the coming payload
 * 6- display data in the cmd
...
typedef enum Packet_Alarm {DEVICE_TYPE,DAC_TYPE ,CHANNEL_ID ,REFERENCE_TIME=3 ,ALARM_STATUS=7 ,DATA_SAMPLE} Packet_Alarm_t; 
...
typedef enum Alarm_State{NO_ALARM=0X00, ALARM_START=0X01 ,ALARM_IN_PROGRESS=0X02 ,ALARM_END=0x03} Alarm_State_t;
...
unsigned char  u8AlarmStatus( const char * p_AlarmStatus);
...
void VDetectDataFromFrame(const char *p_INPUT ,unsigned char *p_output,const unsigned char  LENGTH,Packet_Alarm_t byteIndex);
...
{   /**@Slicing the upcoming frame**/
    ...
    unsigned char dataSample[DATA_SAMPLE];
    dataAqcType=uchDetectAcqmode(Otac+DAC_TYPE); 
    if(dataAqcType==ALARM) 
    {
        /**@Collect data measurments from a BeanDevice in Alarm mode and print it in screen.**/
        printf("**************************************************************************\n");
        ...
        unsigned char alarmStatus= u8AlarmStatus(Otac+ALARM_STATUS);
        VDetectDataFromFrame(Otac ,dataSample,3,DATA_SAMPLE);
        double dataSampleMeasure=dDataSampleMeasured(dataSample);
        VDetectDataFromFrame(Otac ,a_DateUnixFormat ,(4),REFERENCE_TIME);
...
{
 /* Detect Alarm status */
switch (*p_AlarmStatus)
  {
    case NO_ALARM:
      printf("\nAlarm status        :No Alarm  ");
      return NO_ALARM;
    case ALARM_START:
      printf("\nAlarm status        :Alarm Start");   
      return ALARM_START;   
    case ALARM_IN_PROGRESS:
      printf("\nAlarm status        :Alarm in progress");
      return ALARM_IN_PROGRESS;
    case ALARM_END:
      printf("\nAlarm status        :Alarm End");
      return ALARM_END;
    default:
      return -1;
  }
...
{
   /****@Detect the sample measured from the frame

...
public class Fragment_channel_z extends Fragment {

    TextView tv_alarm_chZ;
    LineChart chart;
    long tsLong;
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_channel_z, container, false);
    }
    ...
    public void setUserVisibleHint(boolean isVisibleToUser) {
        ...
        if(isVisibleToUser){
            ...
            handler.postDelayed(new Runnable() {
                @Override
                public void run() {
                    final String ChanneTopic = MqttClient_alarm.deviceId+"/SENSOR/0";
                    tv_alarm_chZ =  (TextView) getView().findViewById(R.id.tv_alarm_chZ);
                    tsLong = System.currentTimeMillis();
                    chart = (LineChart)getView().findViewById(R.id.chart_z);
                        ...
                        public void messageArrived(String topic, MqttMessage mqttMessage) throws UnsupportedEncodingException {
                            if (isAlarm(mqttMessage.getPayload())) {
                                if(topic.equals(ChanneTopic)) {
                                    tv_alarm_chZ.setText(get_alarmStatus(mqttMessage.getPayload()));
